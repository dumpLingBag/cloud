        ...
        <div class="block">
          <vue-scroll>
            <el-tree :data="urlList" :show-checkbox="true" node-key="id" :expand-on-click-node="false"
                     :filter-node-method="filterNode" ref="urlTree" :props="defaultProps" :check-on-click-node="false"
                     @check="checkNodeUrlCheck" :style="{ 'max-height': treeHeight+'px' }">
                  <span class="custom-tree-node" slot-scope="{ node, data }" @mouseenter="commonUrlTree = data.id" @mouseleave="commonUrlTree = 0">
                    <span>{{ data.name }}</span>
