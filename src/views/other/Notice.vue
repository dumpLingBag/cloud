      ...
      <el-table v-loading="loading" element-loading-text="拼命加载中" :data="userList" :height="maxHeight">
        ...
        <el-table-column fixed="right" label="操作" width="300">
          <template slot-scope="scope">
            ...
            <el-button type="danger" size="mini" :disabled="scope.row.account === 'admin'" @click="kickOut(scope.row)">踢出聊天</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination background layout="prev, pager, next" @current-change="noticePage" :current-page="page.currentPage" :total="page.totalSize" :page-size="page.pageSize"></el-pagination>
    </div>
  ...
  methods: {
    ...
    banned (row) { // 禁言
      this.$prompt('请输入禁言时间', '提示', {
        ...
        this.$api.httpPost(this.$url.Notice.banned, this.$qs.stringify({ userId: row.id, expire: value })).then(res => {
          if (res.code === 0) {
